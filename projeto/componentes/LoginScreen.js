...
export default function LoginScreen({ navigation }) {
  ...
  const handleLogin = () => {
    ...
    if (!auth) {
      showModal('Erro Crítico', 'A conexão com o Firebase falhou.');
      return;
    }

    signInWithEmailAndPassword(auth, email, senha)
      .then((userCredential) => {
        showModal('Bem-vindo!', 'Sessão iniciada com sucesso!', true);
      })
      .catch((error) => {
        showModal('Falha no Login', 'E-mail ou senha incorretos.');
      });
  };
